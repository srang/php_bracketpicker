@extends('layouts.app')

@section('content')
@include('common.errors')
@include('common.alerts')
        <div class="bracket-body container">
            <!-- Bracket Form -->
            <div class="row">
                <form action="{{ url('admin/bracket') }}" method="POST">
                    {!! method_field('PUT') !!}
@include('brackets.bracket_form')
                    <div class="pull-left">
                        <a class="btn btn-primary" href="{{ url('/admin/') }}">
                            <i class="fa fa-btn fa-arrow-left"></i> Back
                        </a>
                    </div>
                    <!-- Save Bracket -->
                    <div class="form-group">
                        <div class="pull-right">
                            <button type="submit" class="btn btn-primary">
                                <i class="fa fa-btn fa-pencil"></i> Save
                            </button>
                        </div>
                    </div>
                </form>
            </div>

            <!-- Top Half -->
            <div class="row bracket-row">
                <div class="col-md-2 bracket-round">
                    <h4 class="bracket-round-title">Round 1 {{ $region=$regions->shift()->region }}</h4>
@for($i=1; $i <= 8; $i++)
@include('brackets.bracket_game',[ 'game' => $games->get('1')->shift(), 'game_num' => $i ])
@endfor
                </div>
                <div class="col-md-2 bracket-round">
                    <h4 class="bracket-round-title">Round 2 {{ $region }}</h4>
@for($i=1; $i <= 4; $i++)
@include('brackets.bracket_game',[ 'game' => $games->get('2')->shift(), 'game_num' => $i ])
                <br>
@endfor
                </div>
                <div class="col-md-3 bracket-round">
                    <h4 class="bracket-round-title">Round 3 {{ $region }}</h4>
@for($i=1; $i <= 2; $i++)
@include('brackets.bracket_game',[ 'game' => $games->get('3')->shift(), 'game_num' => $i ])
@endfor
                <br>
                <br>
                    <h4 class="bracket-round-title">Round 3 {{ $region=$regions->shift()->region }}</h4>
@for($i=3; $i <= 4; $i++)
@include('brackets.bracket_game',[ 'game' => $games->get('3')->shift(), 'game_num' => $i ])
@endfor
                </div>
                <div class="col-md-2 bracket-round">
                    <h4 class="bracket-round-title">Round 2 {{ $region }}</h4>
@for($i=5; $i <= 8; $i++)
@include('brackets.bracket_game',[ 'game' => $games->get('2')->shift(), 'game_num' => $i ])
                <br>
@endfor
                </div>
                <div class="col-md-2 bracket-round">
                    <h4 class="bracket-round-title">Round 1 {{ $region }}</h4>
@for($i=9; $i <= 16; $i++)
@include('brackets.bracket_game',[ 'game' => $games->get('1')->shift(), 'game_num' => $i ])
@endfor
                </div>
            </div>

            <!-- Middle -->
            <div class="row bracket-row">
                <div class="col-md-2 bracket-round">
@for($i=1; $i <= 2; $i++)
@include('brackets.bracket_game',[ 'game' => $games->get('4')->shift(), 'game_num' => $i ])
@endfor
                </div>
                <div class="col-md-2 bracket-round">
@include('brackets.bracket_game',[ 'game' => $games->get('5')->shift(), 'game_num' => 1 ])
                </div>
                <div class="col-md-3 bracket-round">
@include('brackets.bracket_game',[ 'game' => $games->get('6')->shift(), 'game_num' => 1 ])
                </div>
                <div class="col-md-2 bracket-round">
@include('brackets.bracket_game',[ 'game' => $games->get('5')->shift(), 'game_num' => 2 ])
                </div>
                <div class="col-md-2 bracket-round">
@for($i=3; $i <= 4; $i++)
@include('brackets.bracket_game',[ 'game' => $games->get('4')->shift(), 'game_num' => $i ])
@endfor
                </div>
            </div>

            <!-- Bottom Half -->
            <div class="row bracket-row">
                <div class="col-md-2 bracket-round">
                    <h4 class="bracket-round-title">Round 1 {{ $region=$regions->shift()->region }}</h4>
@for($i=17; $i <= 24; $i++)
@include('brackets.bracket_game',[ 'game' => $games->get('1')->shift(), 'game_num' => $i ])
@endfor
                </div>
                <div class="col-md-2 bracket-round">
                    <h4 class="bracket-round-title">Round 2 {{ $region }}</h4>
@for($i=9; $i <= 12; $i++)
@include('brackets.bracket_game',[ 'game' => $games->get('2')->shift(), 'game_num' => $i ])
                <br>
@endfor
                </div>
                <div class="col-md-3 bracket-round">
                    <h4 class="bracket-round-title">Round 3 {{ $region }}</h4>
@for($i=5; $i <= 6; $i++)
@include('brackets.bracket_game',[ 'game' => $games->get('3')->shift(), 'game_num' => $i ])
@endfor
                <br>
                <br>
                    <h4 class="bracket-round-title">Round 3 {{ $region=$regions->shift()->region }}</h4>
@for($i=7; $i <= 8; $i++)
@include('brackets.bracket_game',[ 'game' => $games->get('3')->shift(), 'game_num' => $i ])
@endfor
                </div>
                <div class="col-md-2 bracket-round">
                    <h4 class="bracket-round-title">Round 2 {{ $region }}</h4>
@for($i=13; $i <= 16; $i++)
@include('brackets.bracket_game',[ 'game' => $games->get('2')->shift(), 'game_num' => $i ])
                <br>
@endfor
                </div>
                <div class="col-md-2 bracket-round">
                    <h4 class="bracket-round-title">Round 1 {{ $region }}</h4>
@for($i=25; $i <= 32; $i++)
@include('brackets.bracket_game',[ 'game' => $games->get('1')->shift(), 'game_num' => $i ])
@endfor
                </div>
            </div>
        </div>
    </div>

@endsection